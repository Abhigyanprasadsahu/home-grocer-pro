...
const ProductCard = ({ product, quantity, onAdd, onRemove, selectedStores }: ProductCardProps) => {
  ...
  const savings = product.mrp - product.bestPrice;
  ...
  const filteredPrices = selectedStores.length > 0
    ? product.storePrices.filter(sp => selectedStores.includes(sp.store))
    : product.storePrices;
  ...
  const displayPrice = cheapestAvailable?.price || product.bestPrice;
  const displayStore = cheapestAvailable?.store || product.bestStore;
            ...
            {sortedPrices.map((sp) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <span>{sp.logo}</span>
                  <span className="font-medium truncate">{sp.store}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className={cn(
                    "font-bold",
